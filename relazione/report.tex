...
\begin{itemize}
  ...
  \end{cases}
  \]
%  \item $B(t)$: numero di richieste di tipo B completate all'istante t
%  \item $P(t)$: numero di richieste di tipo P completate all'istante t
%  \item $r\textsubscript{B}(t)$: guadagno dovuto alle richieste di tipo
%B all'istante t 
%  \item $r\textsubscript{P}(t)$: guadagno dovuto alle richieste di tipo
%P all'istante t 
%  \item $c(t)$: spese totali dell'attività all'istante t
\end{itemize}

\section{Modello delle specifiche}
%\subsection{Eventi}
%\begin{enumerate}
%  \item Arrivo di una richiesta di tipo B:
%    \begin{itemize}
%      \item $l\textsubscript{B}(t)$ viene incrementato di 1
%      \item Se $l\textsubscript{B}(t) < m \implies$ c'è almeno un server libero \textit{s} che viene
%      selezionato per servire la richiesta e $X\textsubscript{s}(t) \leftarrow
%1$
%    \end{itemize}
%
%  \item Completamento di richiesta di tipo B da parte del servente $s$:
%    \begin{itemize}
%      \item $l\textsubscript{B}(t)$ viene decrementato di 1
%      \item $B(t)$ viene incrementato di 1
%      \item Se $l\textsubscript{B}(t) \geq m \implies$ viene scelto un job dalla coda di B per
%      essere servito, altrimenti $X\textsubscript{s}(t) \leftarrow 0$ 
%    \end{itemize}
%
%  \item Arrivo di una richiesta di tipo P:
%    \begin{itemize}
%      \item $l\textsubscript{P}(t)$ viene incrementato di 1
%      \item Se $l\textsubscript{P}(t) < 2 \implies$ c'è almeno un server libero s che viene
%      selezionato per servire la richiesta e $X\textsubscript{s}(t) \leftarrow
%1$
%    \end{itemize}
%
%  \item Completamento di richiesta di tipo P da parte del servente $s$:
%    \begin{itemize}
%      \item $l\textsubscript{P}(t)$ viene decrementato di 1
%      \item $P(t)$ viene incrementato di 1
%      \item Se $l\textsubscript{P}(t) \geq 2 \implies$ viene scelto un job dalla coda di P per
%      essere servito, altrimenti $X\textsubscript{s}(t) \leftarrow 0$ 
%    \end{itemize}
%
%  \item Campionamento: lo stato non viene alterato ma vengono raccolti i dati
%attuali del sistema
%\end{enumerate}
%
%Si definisce quindi una lista di eventi:
%\begin{center}
%  \begin{tabular}{ |c|c|c|c| }
%    \hline
%    \cellcolor{cellcolor}Indice & \cellcolor{cellcolor}Descrizione & \cellcolor{cellcolor}Attributo 1 & \cellcolor{cellcolor}Attributo 2 \\
...
\newpage
  ...
  \item Le frequenze di uscita medie del singolo server sono indipendenti dalle
fasce orarie e si assumono uguali tra serventi dello stesso tipo e sono le 
seguenti: \newline
